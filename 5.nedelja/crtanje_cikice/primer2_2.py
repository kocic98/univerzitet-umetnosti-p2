...
def add_body_part(surface, attempt):
    ...
    #crtanje jedne ruke
    if attempt >= 7:
        arm = pygame.Surface((50, 5))
        arm.set_colorkey(YELLOW)
        arm.fill(BLUE)
        newarm = pygame.transform.rotate(arm, 30)
    ...
    #crtanje noge
    if attempt >= 9:
        leg = pygame.Surface((50, 8))
        leg.set_colorkey(YELLOW)
        leg.fill(BLUE)
        newleg = pygame.transform.rotate(leg, 30)
        rect = newleg.get_rect()
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            indices = all_indices(guess, presses_key) #[0,2,5]
