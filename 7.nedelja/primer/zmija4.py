...
#klasa za lakse cuvanje pozicije na prozoru
class Point:
...
class Snake(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.tail = []  #cuva gornje leve uglove delova od kojih se sastoji rep
                        #napomena: prvi element u listi ce biti lokacija
                        # poslednjeg dela repa, a poslednji element u listi
                        # ce biti prvi deo do glave pri iscrtavanju
        self.direction =  DOWN
        self.applesNum = 0

    def update(self, apple):

        #dodavanje novog dela na trenutnom mestu glave
        ...
        if (self.rect.left == apple.rect.left) and (self.rect.top == apple.rect.top):
            apple.live = False
            self.applesNum+=1
        else: #ako zmija nije pojela jabuku iz repa se eliminise poslednji deo repa
              # tj. prvi element iz liste tail
            self.tail.pop(0)

    #metod za iscrtavanje delova repa pomocu krugova
    def drawTail(self, where):
        halfStep = STEP_SIZE / 2 #odredjivanje radijusa
        #prolazi se kroz delove repa
        for tailSeg in self.tail:
            #svaki deo se iscrtava pomocu kruga
            pygame.draw.circle(where, (10, 200, 10),
                               (tailSeg.x + halfStep, tailSeg.y + halfStep), #odredjivanje centra kruga,
                                                                             # posto se u listi tail
                                                                             #cuva gornji levi ugao
                               halfStep, 2)
